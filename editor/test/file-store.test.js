...
test('lists only sorted project-relative media files', async (t) => {
  ...
  assert.deepEqual(await listMedia(root), ['media/nested/a.mp3', 'media/z.webp']);
});
...
test('backs up index before replacement', async (t) => {
  ...
  await writeDocumentWithBackup(root, 'new', () => new Date('2026-09-08T12:00:00Z'));
  assert.equal(await fs.readFile(path.join(root, 'index.html.bak-20260908T120000Z'), 'utf8'), 'old');
